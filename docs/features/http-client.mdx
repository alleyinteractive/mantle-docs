...
[`Mixed_Data`](./support/mixed-data.mdx) instance:
...
- `stale`: The amount of time to keep the cached response stale. Accepts a
  number of seconds or a `DateTime` instance. Also accepts a `Closure` that is
  passed a request and response instance to dynamically determine the stale
  time.
- `expire`: The amount of time to keep the cached response before it is
  considered expired. Accepts a number of seconds or a `DateTime` instance. Also
  accepts a `Closure` that is passed a request and response instance to
  dynamically determine the expire time.
