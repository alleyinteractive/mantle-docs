...
outside of the framework and can be installed via Composer:
...
Optionally, you can opt to an exception to be thrown if an error occurs:

```php
try {
  Http::throw_exception()->post(...);
